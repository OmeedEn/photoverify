// PhotoVerify - Chrome Extension Background Service Worker
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({
    ...
    title: "Verify this image with PhotoVerify",
    contexts: ["image"],
  });
});
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "photoverify-check" && info.srcUrl) {
    try {
      ...
      console.error("PhotoVerify error:", error);
      chrome.storage.local.set({
        lastResult: { error: "Failed to verify image. Is the PhotoVerify server running?" },
        lastImageUrl: info.srcUrl,
        lastCheckedAt: new Date().toISOString(),
      });
    }
